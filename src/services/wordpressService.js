...
const todayPosts = async() =>{
    try {
        const today = moment().startOf('day').toISOString();
        const response = await axios.get(process.env.WP_URL + `/wp/v2/posts?per_page=100&status=publish&after=${today}`);
        const posts = response.data;
        const todayPosts = posts.filter(post => moment(post.date).isSame(moment(), 'day'));
        
        const todayPostsCount = todayPosts.length;
        const todayPostURLs = todayPosts.map(post => post.link);
        ...
        console.log("ERROR FETCHING TODAY POSTS");
        return {};   
    }
